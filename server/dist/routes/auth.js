...
auth.post('/forgot-password', async (c) => {
    ...
    const storedAnswers = [
        ...
        user.recoveryAnswer4,
        user.recoveryAnswer5,
    ];
    const hasAnswers = storedAnswers.every(Boolean);
    if (!hasAnswers) {
        return c.json({ error: 'This account has no recovery answers set yet' }, 400);
    }
    const allAnswersMatch = await Promise.all(storedAnswers.map((storedAnswer, index) => bcrypt.compare(normalizeAnswer(answers[index]), storedAnswer)));
