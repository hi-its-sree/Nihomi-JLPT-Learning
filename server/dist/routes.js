...
function normalizeLevelCode(value) {
    ...
    return LEVEL_ORDER.includes(upper) ? upper : 'N5';
}
async function buildLevelCatalog(userId) {
    ...
    return LEVEL_ORDER.map((level, index) => {
        ...
        if (index < currentIndex)
            ...
            progress = Math.max(0, Math.min(100, averageProgress));
...
async function getCumulativeContentTotals(targetLevel) {
    ...
    const sum = (map) => levelsUpToTarget.reduce((total, level) => total + (map[level] ?? 0), 0);
...
async function buildAchievementPayload(userId) {
...
router.get('/api/v1/dashboard', requireAuth, async (c) => {
    ...
    const progressMap = Object.fromEntries(user.progress.map((entry) => [entry.category.toLowerCase(), entry.mastery]));
    const levelOrder = ['N5', 'N4', 'N3', 'N2', 'N1'];
    const targetLevel = user.studyLevel && levelOrder.includes(user.studyLevel)
        ? user.studyLevel
        : 'N5';
...
router.get('/api/v1/vocabulary', requireAuth, async (c) => {
    ...
    if (normalizedStage)
        where.category = normalizedStage;
    if (search) {
        where.OR = [
...
const FLASHCARD_LEVELS = ['N5', 'N4', 'N3', 'N2', 'N1'];
function vocabToCard(v) {
    return {
...
router.get('/api/v1/flashcards', requireAuth, async (c) => {
    ...
    const level = FLASHCARD_LEVELS.includes(queryLevel)
        ? queryLevel
        : (user?.studyLevel ?? 'N5');
    const seenVocabIds = allUserReviews.map((r) => r.vocabularyId).filter((id) => Boolean(id));
    const seenKanjiIds = allUserReviews.map((r) => r.kanjiId).filter((id) => Boolean(id));
...
router.get('/api/v1/progress', requireAuth, async (c) => {
    ...
    const totals = { Kanji: contentTotals.kanji, Vocabulary: contentTotals.vocabulary, Grammar: contentTotals.grammar };
