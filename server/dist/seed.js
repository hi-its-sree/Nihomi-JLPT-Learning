...
async function seedKanji(dataDir) {
...
async function seedVocabulary(dataDir) {
...
async function seedGrammar(dataDir) {
    ...
    for (const line of rows) {
        ...
        const row = Object.fromEntries(header.map((name, index) => [name.trim(), values[index] ?? '']));
